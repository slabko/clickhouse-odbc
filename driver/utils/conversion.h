...
// This is very inconvenient when writing C++ code, since we typically use `std::string`
// and `std::u16string` for strings. Defining `std::basic_string<unsigned char>` or
// `std::basic_string<unsigned short>` is not only verbose, but generally not recommended.
// For example, libc++ removed `std::char_traits` specializations for such character types.
//
// Therefore, it is much easier to use standard-defined string types and perform casts
// from signed character pointers (e.g., `char*`) to unsigned ones (e.g., `SQLCHAR*`)
// only when actually calling ODBC functions.
//
// The purpose of the code below is to define universal approach for creating and passing strings to ODBC functions.
//
...
// It is similar to SQLTCHAR (SQL Text CHARacter) but instead of using
// unsigned types which are not very well supported by the C++ standard, it
// defined in terms of well defined signed types.
// Here is an example of why using signed char types for strings is a bad idea:
// https://github.com/llvm/llvm-project/blob/release/17.x/libcxx/docs/ReleaseNotes/17.rst#upcoming-deprecations-and-removals
// The name PTChar looks ugly but it is very distinct from other types, such as char, or SQLTCHAR,
// and that distinction is a goal for this type.
#ifdef UNICODE
    using PTChar = char16_t;
#else
    using PTChar = char;
#endif
...
// Casts C++ string char pointer types, i.e. char * and char16_t *, to ODBC's unsigned pointer types,
...
// Windows' LPSTR, LPWSTR, etc. do not have this property. LPSTR, aka Long Pointer STRing is a `char*`, while
// LPWSTR, aka Long Pointer Wide STRing, is `wchar_t *` on windows and `unsigned short*` on unixODBC.
// The LPTSTR alias, aka Long Pointer Text String, is `unsigned char*` when UNICODE is not defined, and
// `unsigned short *` when it is defined. We need, however a type to which we can convert the content of our strings
// when calling functions that expect these types, such as SQLGetPrivateProfileString, SQLWriteDSNToIni, etc.
// This is shortest and easiest common denominator that works.
#ifdef UNICODE
