...
TEST_F(MiscellaneousTest, ClickhouseToSQLTypeMapping)
{
    ...
    std::vector<TypeMappingTestEntry> types = {
        ...
        // Non-string LowCardinality columns are supported by RowBinaryWithNamesAndTypes
        // but they are all represented as SQL_VARCHAR
    ...
    for(const auto [type, input, sql_type] : types) {
        ...
        query_stream << std::format(" CAST({}, '{}') as `{}`,", input, type, type);
    }
