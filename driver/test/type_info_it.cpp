...
public:
    static void execute(const std::string& query)
    {
        ...
        ODBC_CALL_ON_DBC_THROW(dbc, SQLConnect(dbc, dsn_wptr, SQL_NTS, nullptr, 0, nullptr, 0));
        ODBC_CALL_ON_DBC_THROW(dbc, SQLAllocHandle(SQL_HANDLE_STMT, dbc, &stmt));

        auto wquery = fromUTF8<SQLTCHAR>(query);
...
std::string SQLColumnsResultTest::databaseName{"test_suite"};
...
TEST_F(SQLColumnsResultTest, SimpleTable)
{
    ...
    std::string database_name = getDatabaseName();

    size_t pos = 1;
    std::stringstream query_stream;
    query_stream << "create table " << database_name << "." << table_name << "(\n";
    ...
    auto table = fromUTF8<SQLTCHAR>("simple_table");
    ODBC_CALL_ON_STMT_THROW(hstmt, SQLColumns(
        hstmt,
